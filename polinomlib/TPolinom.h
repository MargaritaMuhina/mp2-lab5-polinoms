...
void TPolinom::AddMonom(TMonom m)
{
	...
	int ind1 = m.GetIndex();
	if (ind1 > ind)InsertFirst(m); else
	{
		while (ind1 < ind) {
			if (i < sz) {
				GoNext();
...

{
	...
	TPolinom* result = new TPolinom(*this);  // —оздаем копию текущего полинома
	TPolinom tempOther(other);  // —оздаем копию переданного полинома
